...
namespace CustomAccessRefreshApi
{
    public static class IdentityServer4Client
    {
        ...
        private static string _stsUrl = "https://localhost:5010";
        ...
        public static async Task<TokenResponse> LoginAsync(string user, string password)
        {
            ...
            _disco = await HttpClientDiscoveryExtensions.GetDiscoveryDocumentAsync(_httpClient, _stsUrl);
        ...
        private static void ShowResponse(TokenResponse response)
        {
            if (!response.IsError)
            {
                ...
                if (response.AccessToken.Contains("."))
                {
                    ...
                    Log.Logger.Debug("Access Token Header decoded {AccessHeader}", 
                        JObject.Parse(Encoding.UTF8.GetString(Base64Url.Decode(header))).ToString());

                    Log.Logger.Debug("Access Token claims decoded {Claims}", 
                        JObject.Parse(Encoding.UTF8.GetString(Base64Url.Decode(claims))).ToString());
                }
            }
            else
            {
